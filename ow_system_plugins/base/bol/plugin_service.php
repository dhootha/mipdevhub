...
class BOL_PluginService
{
    ...
    public function checkUpdates()
    {
        ...
        {
            $requestArray[] = array('key' => $plugin->getKey(), 'developerKey' => $plugin->getDeveloperKey(), 'build' => $plugin->getBuild());
        }

        $requestUrl = OW::getRequest()->buildUrlQueryString(self::UPDATE_SERVER . 'get-update-info/', array('plugins' => json_encode($requestArray)));

        $resultArray = json_decode((file_get_contents($requestUrl)));
        ...
        foreach ( $plugins as $plugin )
        {
            if ( in_array($plugin->getKey(), $resultArray) && (int) $plugin->getUpdate() === 0 )
            {
                $plugin->setUpdate(1);
        ...
        if ( in_array('core', $resultArray) )
        {
            OW::getConfig()->saveConfig('base', 'update_soft', 1);
        }
    }

    public function getPluginInfoForUpdate( $key, $devKey )
    {
        $requestUrl = OW::getRequest()->buildUrlQueryString(self::UPDATE_SERVER . 'get-plugin-info', array('key' => trim($key), 'developerKey' => $devKey));
    ...
    {
        return (array) json_decode((file_get_contents(self::UPDATE_SERVER . 'platform-info')));
    }

    public function downloadCore( $archivePath )
    {
        $requestUrl = OW::getRequest()->buildUrlQueryString(self::UPDATE_SERVER . 'download-platform', array(
    ...
    public function downloadPlugin( $key, $devKey, $licenseKey = null )
    {
        $requestUrl = OW::getRequest()->buildUrlQueryString(self::UPDATE_SERVER . 'get-plugin', array('key' => trim($key), 'developerKey' => trim($devKey), 'licenseKey' => $licenseKey));
    ...
    {
        $requestUrl = OW::getRequest()->buildUrlQueryString(self::UPDATE_SERVER . 'check-license-key', array('key' => trim($key), 'licenseKey' => $licenseKey, 'developerKey' => $developerKey));
