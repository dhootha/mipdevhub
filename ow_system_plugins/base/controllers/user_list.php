...
class BASE_CTRL_UserList extends OW_ActionController
{
    ...
    private function getData( $listKey, $first, $count )
    {
        ...
        switch ( $listKey )
        {
            case 'latest':
                return array(
                    $service->findList($first, $count),
                    $service->count()
                );

            case 'online':
                return array(
                    $service->findOnlineList($first, $count),
                    $service->countOnline()
                );
            ...
            case 'featured':
                ...
                return array(
                    $service->findFeaturedList($first, $count),
                    $service->countFeatured()
                );

            case 'waiting-for-approval':
                return array(
                    $service->findDisapprovedList($first, $count),
                    $service->countDisapproved()
                );
